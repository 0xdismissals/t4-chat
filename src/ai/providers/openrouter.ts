...
export class OpenRouterProvider {
  ...
  async *generateTextStream(
    ...
    // Gracefully handle non-image attachments. The OpenAI-compatible Chat Completions 
    // API used here only supports image MIME types.
    const latestMessage = messages[messages.length - 1];
    if (latestMessage.attachment && !latestMessage.attachment.type.startsWith('image/')) {
        yield { 
            type: 'paragraph', 
            content: `I'm sorry, but this model's API integration in T4-Chat currently only supports image file uploads, not files of type \`${latestMessage.attachment.type}\`. The ability to analyze documents requires a different implementation.` 
        };
        return;
    }
    ...
    try {
      ...
      try {
        ...
        yield {
          type: 'paragraph',
          content: `An error occurred while communicating with the OpenRouter API. Please check your API key and network connection. \n\n**Details:** ${errorMessage}`
        };
        return;
    }
  }
